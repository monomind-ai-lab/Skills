...
| Start every implementation task in a new worktree from `origin/main` | Adopted as the Monomind factory default. A harness-managed worktree qualifies when exclusive and base-verified; a repository that cannot provide `origin/main` needs a recorded exception. Never build on `main`. |
